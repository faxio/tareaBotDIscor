...
async def on_ready():
    ...
    channel = bot.get_channel(876897627369799744)#913706828502814760
...
@bot.command(name='b', help='Buscar en wikipedia')
async def busqueda(ctx,*,message):
    print("send a new mesage to rabbitmq: "+message)
    embed = discord.Embed(title=f"Buscando Informacion sobre: {message}", color=discord.Color.blue())
    await ctx.send(embed=embed)

    channelMQ.basic_publish(exchange='cartero', routing_key="search", body=message)

@bot.command(name='t', help="Traduce al español el texto escrito ")   
async def traducir(ctx,*,message):
    print("send a new mesage to rabbitmq: "+message)
    
    embed = discord.Embed(title=f"Traduciendo Texto: {message}", color=discord.Color.blue())
    await ctx.send(embed=embed)

    channelMQ.basic_publish(exchange='cartero', routing_key="translate", body=message)
############ CONSUMER ###############
